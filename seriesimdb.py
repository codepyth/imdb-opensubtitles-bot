...
def get_movie_details(imdb_id, season=None):
    imdb_numeric_id = imdb_id[2:]
    if season:
        url = f"https://www.opensubtitles.org/en/search/sublanguageid-ara,eng,fre/pimdbid-{imdb_numeric_id}/season-{season}"
    else:
...
def choose_season(imdb_id):
    ...
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        seasons = soup.find_all('a', href=True, title=re.compile(r'Season'))
        if seasons:
            print("Available Seasons:")
            for idx, season in enumerate(seasons):
                season_title = season['title']
                season_number = season_title.lower().replace('season', '').strip()
                print(f"{idx + 1}. {season_title} ({season_number})")
            
            season_choice = int(input("Choose a season number: ")) - 1
            chosen_season = seasons[season_choice]['title'].lower().replace('season', '').strip()
            return chosen_season
        else:
            print("No seasons found.")
            return None
    else:
        print(f"Failed to retrieve seasons for IMDb ID {imdb_id}.")
        return None
...
def main():
    ...
    season = choose_season(imdb_id)
    movie_title, rows = get_movie_details(imdb_id, season)
